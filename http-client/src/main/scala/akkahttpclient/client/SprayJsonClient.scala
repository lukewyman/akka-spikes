...
/**
  ...
  */
object SprayJsonClient extends App {

  implicit val system = ActorSystem()
  implicit val ec = system.dispatcher

  val cc = new SprayCarClient
...
trait SpraySerializationSupport extends SprayJsonSupport with DefaultJsonProtocol {

  implicit val ApiStatusFormat = jsonFormat2(Car)
}

class SprayCarClient(implicit val system: ActorSystem) extends SpraySerializationSupport {
  import SprayCarClient._
...
object SprayCarClient {
  type ClientResult[T] = Either[SprayCarClientError, T]

  sealed trait SprayCarClientError
  case class NotFound(error: String) extends SprayCarClientError
  case class BadRequest(error: String) extends SprayCarClientError
  case class Unauthorized(error: String) extends SprayCarClientError
  case class UnexpectedStatusCode(statusCode: StatusCode) extends SprayCarClientError
}
